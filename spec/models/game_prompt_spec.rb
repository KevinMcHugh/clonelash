...
RSpec.describe GamePrompt do
  ...
  let(:game_prompt) { described_class.create(game: game, prompt: prompt, state: :accepting_votes) }
  ...
  describe '#as_json' do
    ...
    context 'prompt is not accepting_votes' do
      ...
      context 'player is playing,' do
        ...
        context 'player has not voted,' do
          context 'final question' do
            ...
            let(:game_prompt) { described_class.create(game: game, prompt: prompt, final_question: true, state: :accepting_votes) }
          ...
          context 'not final question,' do
            ...
            context 'player has not responded' do
              it 'says all responses are selectable' do
                expect(subject[:responses].map { |r| r[:selectable] }).to eq([true,true])
              end
            end
          end
        end
